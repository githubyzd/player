...
void *audio_decode(void *args) {
    LOGI("Method start---> AudioChannel audio_decode");
    AudioChannel *audioChannel = static_cast<AudioChannel *>(args);
    audioChannel->decode();
    LOGI("Method end---> AudioChannel audio_decode");
    return 0;
}

void *audio_play(void *args) {
    LOGI("Method start---> AudioChannel audio_play");
    AudioChannel *audioChannel = static_cast<AudioChannel *>(args);
    audioChannel->_play();
    LOGI("Method end---> AudioChannel audio_play");
    return 0;
}
...
void AudioChannel::play() {
    LOGI("Method start---> AudioChannel play");
    packets.setWork(1);
    frames.setWork(1);
    ...
    LOGI("Method end---> AudioChannel play");
}


void AudioChannel::decode() {
    LOGI("Method start---> AudioChannel decode");
    AVPacket *packet = 0;
    while (isPlaying) {
        //取出一个数据包
    ...
    releaseAvPacket(&packet);
    LOGI("Method end---> AudioChannel decode");
}
...
int AudioChannel::getPcm() {
    LOGI("Method start---> AudioChannel getPcm");
    int data_size = 0;
    AVFrame  *frame;
...
void AudioChannel::_play() {
    LOGI("Method start---> AudioChannel _play");
    /**
  * 1、创建引擎并获取引擎接口
  */
    SLresult result;
